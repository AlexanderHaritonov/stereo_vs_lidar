...
def get_fx_and_baseline(calib_file):
...
def get_q_matrix(calib_file):
    """Disparity-to-depth mapping matrix (Q), derived via cv2.stereoRectify from the
    raw (unrectified) intrinsics/extrinsics of cam2 and cam3 -- independent of the
    P_rect-based fx/baseline shortcut in get_fx_and_baseline()."""
    calib = read_calib_file(calib_file)
    ...
    # R_0i/T_0i are each camera's pose relative to cam0 -- compose them to get the
    # relative pose from cam2 to cam3, which is what stereoRectify needs.
    R_23 = R_03 @ R_02.T
    T_23 = T_03 - R_23 @ T_02
